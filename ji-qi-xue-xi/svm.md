...
那么此时如何衡量样本属于正例还是负例呢？这里使用的是距离的方式来衡量，$$\frac{|wx + b|}{\left \| w \right \|}$$就可以得到样本到分界面$$w^Tx + b = 0$$的距离。如果该样本属于正例的话，那么wx + b的结果为一个正数，如果是负例的话，则是一个负数，那么这个时候如果我们给wx + b乘上标记值y，那就可以去掉wx + b的绝对值符号了，结果为y\*\(wx + b\) / \|w\|, 这就是最终的距离衡量式子。
