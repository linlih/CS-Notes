...
这里使用的是距离的方式来衡量，$$\frac{|wx + b|}{\left \| w \right \|}$$就可以得到样本到分界面$$w^Tx + b = 0$$的距离。如果该样本属于正例的话，那么$$w^Tx + b$$的结果为一个正数，如果是负例的话，则是一个负数，那么这个时候如果我们给$$w^Tx + b$$乘上标记值$$y$$，当样本点落在直线上方的时候，距离为正值，对应的标记值$$y$$为+1，。如果样本点位于直线下方，距离为负值，对应的标记值$$y$$为-1，二者相乘也为正值。那就可以去掉$$w^Tx + b$$的绝对值符号了，结果为$$ \frac { y*(wx + b)} {\left \| w \right \|} $$, 这就是最终的距离衡量式子。
