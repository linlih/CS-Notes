...
即将$$(2, 1)$$作为$$w$$，$$\begin{pmatrix}x \\ y\end{pmatrix} $$作为$$x$$，$$-4$$作为$$b$$，这样分界面就可以写成$$w^T x + b = 0$$。其中$$w$$和$$x$$为向量，$$b$$为标量。

此时将样本点带入$$w^Tx + b$$中得到的结果如果为正数，样本点就位于直线法线的正向，反之则位于法线的负向。

那么此时如何衡量样本属于正例还是负例呢？

这里使用的是距离的方式来衡量，$$\frac{|wx + b|}{\left \| w \right \|}$$就可以得到样本到分界面$$w^Tx + b = 0$$的距离。如果该样本属于正例的话，那么$$w^Tx + b$$的结果为一个正数，如果是负例的话，则是一个负数，那么这个时候如果我们给$$w^Tx + b$$乘上标记值$$y$$，当样本点落在直线上方的时候，距离为正值，对应的标记值$$y$$为+1，二者相乘为正值。如果样本点位于直线下方，距离为负值，对应的标记值$$y$$为-1，二者相乘也为正值。那就可以去掉$$w^Tx + b$$的绝对值符号了，结果为$$ \frac { y*(wx + b)} {\left \| w \right \|} $$, 这就是最终的距离衡量式子。

那么我们要的目标是什么呢？

我们希望的是能够找到一个分割面，二维的话就是一条直线，能够使得样本离这条直线的距离最大。那么在训练过程中，我们需要保证的是，所有样本中离分界平面最小的那个样本点，这个样本点离分界平面的距离达到最大，这样得到的分界平面就是最优的。

目标函数：$$arg\, {\underset {w}{\operatorname {max} }}\,  {\underset {w}{\operatorname {min} }}\, \frac { y*(wx + b)} {\left \| w \right \|} $$
...
那么目标函数就可以化简为：$$arg\, {\underset {w}{\operatorname {max} }}\,   \frac{1}{\left \|w \right ||}$$

此时我们多了一个约束条件，$$ y*(wx + b)$$大于等于1，由上面求的是$$ argmax\ min(y * (wx + b))$$,那么这个时候如果$$ y*(wx + b)$$满足大于等于1的话，那么它取最最小值就是1了
...
* \[4\]化学狗码砖的日常. \[知乎\]\([https://zhuanlan.zhihu.com/p/29212107](https://zhuanlan.zhihu.com/p/29212107)\)
* 
