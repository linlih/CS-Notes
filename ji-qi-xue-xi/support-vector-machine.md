...
如果该样本属于正例的话，那么$$w^Tx + b$$的结果为一个正数，如果是负例的话，则是一个负数，那么这个时候如果我们给$$w^Tx + b$$乘上标记值$$y$$，当样本点落在直线上方的时候，距离为正值，对应的标记值$$y$$为+1，**二者相乘为正值**。如果样本点位于直线下方，距离为负值，对应的标记值$$y$$为-1，**二者相乘也为正值**。那就可以去掉$$w^Tx + b$$的绝对值符号了，结果为$$ \frac { y*(wx + b)} {\left \| w \right \|} $$, 这就是最终的距离衡量式子。
...
$$
{\underset {w, b}{arg\, \operatorname {max} }}\,\{  \frac { 1} {\left \| w \right \|}{\underset {n} {\operatorname {min} }}\,  y_n*(wx_n + b)\}
$$
...
那么目标函数就可以化简为：$$arg\, {\underset {w}{\operatorname {max} }}\,   \frac{1}{\left \|w \right ||}\\ s.t. \ \  y_n*(wx_n + b) \geq 1$$
...
$$
arg\, {\underset {w}{\operatorname {min} }}\,    \frac{1}{2} \left \| w \right \|^2\\ s.t. \ \  y_n*(wx_n + b) \geq 1
$$
